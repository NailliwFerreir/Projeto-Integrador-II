...
//Mensagem "Bilhete expirado!"
function MessageError() {
  let activateMessage = document.getElementById("successActivateMessage");
  activateMessage.innerHTML = `<p id="successMessage" class="activeTicketXprd">Bilhete Expirado!</p>`;
}
let code = document.getElementById("ticketCodeVerification").value;
//Verifica as recargas para fazer a utilização
function searchRecharge() {
  let code = document.getElementById("ticketCodeVerification").value;
  let codeManager = document.getElementById("ticketCodeVerificationManage").value;
  let SearchUrl = `http://localhost:3000/Recarga/${code}`;
...
//função que envia par ao backend os dados para a utilização
 function UtiliRec() {
...
function searchHist() {
  ...
  let gerenUrl = `http://localhost:3000/Gerenciamento/${cdb}`;
  axios
  .get(gerenUrl)
  .then((response) => {
  ...
  .catch((error) => {
    ...
    const createlistaGerenciamento = (Utilizados) => {
      const ulHistor = document.getElementById("extract");
      Utilizados.map((utilizados) => {
        const listHistor = document.createElement("li"); 
        listHistor.innerHTML =`${utilizados.anything} ${utilizados.data}`;
